...
public class Hw2 {
	public static void main(String[] args) throws SQLException {
		...
		try {
			...
			connection = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl1", "hr", "orcl1");
		...
		if (queryType.equals("within")) {
			...
			String query1 = "SELECT t.TRAMSTATION_ID FROM TRAMSTOP t,student s WHERE s.student_id='" + student_id + "' "
					+ "and SDO_WITHIN_DISTANCE(t.shape,s.shape," + "'distance=" + distance + "') = 'TRUE' ";
			String query2 = "SELECT b.building_id FROM building b,student s WHERE s.student_id='" + student_id
					+ "' and " + "SDO_WITHIN_DISTANCE(b.shape,s.shape,'distance=" + distance + "') = 'TRUE'";
			try {
		...
		if (queryType.equals("nearest-neighbor")) {
			...
			if (args[1].equalsIgnoreCase("student")) {
				...
				query = "SELECT s1.student_id FROM student s1,student s2 WHERE"
						+ " SDO_NN(s1.shape,s2.shape, 'sdo_num_res=" + number1 + "') = 'TRUE'and s2.student_id='"
						+ args[2] + "' and s1.student_id<>'" + args[2] + "'";

			} else if (args[1].equalsIgnoreCase("building")) {
				int number1 = number + 1;
				query = "SELECT b1.building_id FROM building b1,building b2 WHERE"
						+ " SDO_NN(b1.shape,b2.shape, 'sdo_num_res=" + number1 + "') = 'TRUE'and  b2.building_id='"
						+ args[2] + "'" + " and b1.building_id <>'" + args[2] + "'";

			} else {
				int number1 = number + 1;
				query = "SELECT t1.tramstation_id FROM tramstop t1,tramstop t2 WHERE"
						+ " SDO_NN(t1.shape,t2.shape, 'sdo_num_res=" + number1 + "') = 'TRUE'and t2.tramstation_id='"
						+ args[2] + "' and t1.tramstation_id<>'" + args[2] + "'";
		...
		if (queryType.equals("fixed")) {
			if (args[1].equalsIgnoreCase("1")) {
				...
				String query = "select s.student_id " + "from student s, tramstop t "
						+ "WHERE SDO_RELATE(s.shape,sdo_geometry(2003,NULL,NULL, " + "sdo_elem_info_array(1,1003,4), "
						...
						+ ")),'mask=ANYINTERACT') = 'TRUE' and t.tramstation_id = 't2ohe' " + "intersect "
						+ "select s.student_id " + "from student s, tramstop t "
						+ "WHERE SDO_RELATE(s.shape,sdo_geometry(2003,NULL,NULL, " + "sdo_elem_info_array(1,1003,4), "
						+ "sdo_ordinate_array(t.shape.sdo_point.x+t.tram_radius,t.shape.sdo_point.y, t.shape.sdo_point.x,t.shape.sdo_point.y+t.tram_radius, "
						+ "t.shape.sdo_point.x-t.tram_radius,t.shape.sdo_point.y "
						+ ")),'mask=ANYINTERACT') = 'TRUE' and t.tramstation_id = 't6ssl' " + "union " +

				"select b.building_id " + "from building b, tramstop t "
						+ "WHERE SDO_RELATE(b.shape,sdo_geometry(2003,NULL,NULL, " + "sdo_elem_info_array(1,1003,4), "
						+ "sdo_ordinate_array(t.shape.sdo_point.x+t.tram_radius,t.shape.sdo_point.y, t.shape.sdo_point.x,t.shape.sdo_point.y+t.tram_radius, "
						+ "t.shape.sdo_point.x-t.tram_radius,t.shape.sdo_point.y)),'mask=ANYINTERACT') = 'TRUE' and t.tramstation_id = 't2ohe' "
						+ "intersect " + "select b.building_id " + "from building b, tramstop t "
						+ "WHERE SDO_RELATE(b.shape,sdo_geometry(2003,NULL,NULL, " + "sdo_elem_info_array(1,1003,4), "
						+ "sdo_ordinate_array(t.shape.sdo_point.x+t.tram_radius,t.shape.sdo_point.y, t.shape.sdo_point.x,t.shape.sdo_point.y+t.tram_radius, "
			...
			if (args[1].equalsIgnoreCase("2")) {
				String query = "select s.student_id,t.tramstation_id" + " from tramstop t, student s"
			...
			if (args[1].equalsIgnoreCase("4")) {
				String query = "select * from (" + " select s.student_id ,count(s.student_id)"
						...
						+ " ) group by s.student_id order by count(s.student_id) desc)" + " where rownum<6 ";
				int count = 0;
				try {
			...
			if (args[1].equalsIgnoreCase("5")) {
				String query = " SELECT MIN(lower_x)AS lower_left_x," + " MIN (lower_y)    AS lower_left_y ,"
						+ " MAX(upper_x)     AS upper_right_x," + " MAX(upper_y)     AS upper_right_y" + " FROM"
						+ "(select" + " SDO_GEOM.SDO_MIN_MBR_ORDINATE(BUILDING.shape, M.DIMINFO,1) AS LOWER_X,"
						...
						+ " SDO_GEOM.SDO_MAX_MBR_ORDINATE(BUILDING.shape, M.DIMINFO,2) AS UPPER_Y" + " FROM BUILDING ,"
						+ " USER_SDO_GEOM_METADATA M" + " WHERE M.TABLE_NAME = 'BUILDING'"
						+ " AND M.COLUMN_NAME  = 'SHAPE'" + " AND BUILDING.BUILDING_NAME LIKE upper('SS%')" + " )";
				try {
					...
					while (rSet.next()) {
						...
						System.out.println("lower_x : " + lower_x + " lower_y :" + lower_y + " upper_x :" + upper_x
								+ " upper_y :" + upper_y);
					}
				} catch (SQLException e) {
